...
import edu.caltech.cs141b.hw5.gwt.collab.shared.DocumentException;
...
import edu.caltech.cs141b.hw5.gwt.collab.shared.Parameters;
...
public class CollaboratorServiceImpl extends RemoteServiceServlet implements
	...
	/**
	 * Same as above function, but with ip 
	 */
		...
		try{
			...
			if(doc.getKey() != null)
			{
				...
				if(!document.isLocked())
					throw new LockExpired("saveDocument(): " +
							"Lock expired, so cannot save.");
				else if(!document.getLockedBy().equals(ip))
					throw new LockExpired("saveDocument(): Another user has " +
							"acquired the lock, so cannot save.");
				else
				{
